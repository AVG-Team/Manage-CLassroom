<x-admin.layouts.app>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('admin.default-salary.edit', $defaultSalary->id) }}
    @endsection
    <div>
        <div class="flex flex-col gap-9">
            ...
            >
                ...
                <form action="{{ route("admin.default-salary.update", $defaultSalary) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="p-6.5">
                        <div class="mb-4.5">

                            <label for="salary" class="block text-sm font-medium mb-2 dark:text-white">Lương Cơ Bản</label>
                            <div class="relative">
                                <input type="text" id="salary" name="salary" value="{{ old('salary') ?: $defaultSalary->salary }}"
                            ...
                            @if($errors->has('salary'))
                                <p class="text-red-500 mt-2 text-sm">{{ $errors->first('salary') }}</p>
                            @endif
                        </div>
